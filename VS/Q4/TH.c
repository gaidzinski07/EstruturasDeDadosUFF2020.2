...
int TH_retira(char *tabHash, char *dados, int tam, int num){
  FILE *fp = fopen(tabHash,"rb");
  if(!fp) exit(1);
  int pos, h = TH_hash(num, tam);
  fseek(fp, h*sizeof(int), SEEK_SET);
...
void TH_insere(char *tabHash, char *dados, int tam, int num){
  ...
  while(pos != -1){
    ...
    if((!aux.cont) && (prim_pos_livre == -1))prim_pos_livre=pos;
    ant = pos;
    pos = aux.prox;
  }
  if(prim_pos_livre == -1){
...
void TH_imprime (char *tabHash, char *dados, int tam){
  ...
  for(i = 0; i < tam; i++){
    ...
    while(p != -1){
      ...
      printf("$%d: num: %d\tcont: %d\tprox_end: $%d\n", pos, x.num, x.cont, x.prox);
      p = x.prox;
    }
  }
  fclose(fp);
